...
class Bot {
  constructor(options) {
    ...
    this.relayChannelJoins = options.relayChannelJoins !== true; // default to false
    // Parts include 'part' and 'kick'
    this.relayChannelParts = options.relayChannelParts !== true; // default to false
    // Quits include 'quit' and 'kill'
    this.relayChannelQuits = options.relayChannelQuits !== true; // default to false
  ...
  attachListeners() {
    ...
    this.ircClient.on('join', (channel, nick, message) => {
      if(this.relayChannelJoins) {
        this.sendToDiscord(nick + '/join', channel, `Joined ${channel}: ${reason}`);
      }
    });

    this.ircClient.on('part', (channel, nick, reason, message) => {
      if(this.relayChannelParts) {
        this.sendToDiscord(nick + '/part', channel, `Parted ${channel}: ${reason}`);
      }
    });

    this.ircClient.on('kick', (channel, nick, by, reason, message) => {
      if(this.relayChannelParts) {
        this.sendToDiscord(nick + '/kick', channel, `Kicked from ${channel} by ${by}: ${reason}`);
      }
    });
    ...
    this.ircClient.on('quit', (nick, reason, channels, message) => {
      if(this.relayChannelQuits) {
        for (var i = 0; i < channels.length; i++) {
          this.sendToDiscord(nick + '/quit', channels[i], `Quit: ${reason}`);
        }
      }
    });
    ...
    this.ircClient.on('kill', (nick, reason, channels, message) => {
      if(this.relayChannelQuits) {
        for (var i = 0; i < channels.length; i++) {
          this.sendToDiscord(nick + '/kill', channels[i], `Killed: ${reason}`);
        }
      }
    });
  }
